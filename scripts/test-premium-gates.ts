...
run('set 1 free for guest and signed-in until demo used', () => {
  assert.equal(tieredSetQuizPageAccess(1, false, false, false), 'allow')
  assert.equal(tieredSetQuizPageAccess(1, true, false, false), 'allow')
  assert.equal(tieredSetQuizPageAccess(1, false, false, true), 'require_sign_in')
  assert.equal(tieredSetQuizPageAccess(1, true, false, true), 'require_premium')
})

run('set 2+ sign-in then premium', () => {
...
run('mdcatMockPageAccess mirrors set rules', () => {
  assert.equal(mdcatMockPageAccess(1, false, false, false), 'allow')
  assert.equal(mdcatMockPageAccess(1, true, false, false), 'allow')
...
run('tieredSetTableNavigation', () => {
  ...
  assert.equal(tieredSetTableNavigation(1, true, false, false), 'navigate')
...
run('examDashboardMockClick', () => {
  ...
  assert.equal(examDashboardMockClick(1, true, false, false), 'open')
