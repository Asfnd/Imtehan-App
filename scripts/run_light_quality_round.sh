...
RESUME_ARGS=()
if [[ "${2:-}" == "--resume" ]] || [[ "${AGENT_RESUME:-0}" == "1" ]]; then
  RESUME_ARGS=(--resume)
fi

echo "→ Light quality plan: $PLAN"
echo "  clamp=$AGENT_CLAMP_BATCH_SIZE lite=$AGENT_LITE_MODE metrics=$AGENT_BATCH_METRICS min_yield=$AGENT_MIN_YIELD_RATIO pulse_every=${AGENT_QC_PULSE_EVERY} cooldown=${AGENT_COOLDOWN_SEC}s resume=${RESUME_ARGS[*]:-off}"
exec python3 scripts/agent_mcq_pipeline.py run --plan "$PLAN" --workers "${AGENT_MAX_WORKERS}" "${RESUME_ARGS[@]}"
