...
async function createTable() {
  ...
  if (error) {
    console.log('⚠️  Table does not exist. Please run this SQL in Supabase:')
    console.log(`
...
async function insertDatabaseRecord(paper) {
  const { error } = await supabase
    .from('guess_papers_2026')
    .upsert({
...
async function main() {
  ...
  if (!tableExists) {
    console.error('❌ Please create the table first using the SQL above.')
