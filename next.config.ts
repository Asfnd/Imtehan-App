...
const nextConfig: NextConfig = {
  ...
  async redirects() {
    return [
      ...
      // Escape CF-poisoned /mock, /mock-attempt, /attempt HTML — live path is /run/:id
      {
        source: '/exams/:exam/mock/:mockId(\\d+)',
        destination: '/exams/:exam/run/:mockId',
        permanent: false,
      },
      {
      ...
      {
        source: '/exams/:exam/attempt/:mockId(\\d+)',
        destination: '/exams/:exam/run/:mockId',
        permanent: false,
      },
  ...
  async headers() {
    return [
      ...
      {
        source:
          '/((?!api|auth|profile|admin|signin|community|quiz|mpt-practice|css/css-practice|css/solved-papers/view|css/past-papers/view|css/guess-papers/view|exams/.+/run|exams/.+/attempt|exams/.+/mock-attempt|exams/.+/mocks|exams/.+/mock|_next/static|_next/image|favicon.ico).*)',
