...
const LEVEL_CONFIG: Record<string, { label: string; badgeClass: string; iconClass: string }> = {
...
export default function DifficultySetPicker() {
  ...
  useEffect(() => {
    async function fetchCount() {
      ...
      if (!section || !LEVEL_CONFIG[level]) { setLoading(false); return }
  ...
  const handleSetClick = (setNum: number) => {
    ...
    router.push(`/exams/${examSlug}/${subjectSlug}/difficulty/${level}/set/${setNum}`)
  }
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4 max-w-7xl pt-3 pb-1">
          <Breadcrumb items={[
            ...
            { name: `${levelConfig.label} Questions`, url: `/exams/${examSlug}/${subjectSlug}/difficulty/${level}` },
          ]} />
        </div>

        <div className="container mx-auto px-4 py-8 max-w-7xl">
          <div className="mb-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-blue-500 mb-1">Difficulty Practice</p>
            <div className="flex items-center gap-2.5">
              <h1 className="text-xl font-semibold text-gray-900">{levelConfig.label} Questions</h1>
              <span className={`rounded-full border px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide ${levelConfig.badgeClass}`}>
                {levelConfig.label}
              </span>
            </div>
          ...
          {totalSets === 0 ? (
            <div className="text-center py-16 text-gray-500">
              <p className="text-lg font-medium mb-2">No MCQs found for this difficulty level</p>
              <button
