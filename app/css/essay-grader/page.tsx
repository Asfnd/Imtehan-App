...
export default function WritingCoachPage() {
  ...
  // Long answer fields
  const [laSubject, setLaSubject] = useState(CSS_SUBJECTS[0])
  ...
  const canSubmit = () => {
    if (mode === 'essay') return essayTopic.trim().length > 0 && wordCount(essayContent) >= 200
  ...
  const submitHint = () => {
    if (mode === 'essay') {
      ...
      if (wc < 200) return `Write at least 200 words (${wc} written) — CSS essays require 1000–1200`
    }
    if (mode === 'precis') {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-slate-800 via-blue-900 to-slate-800 border-b border-blue-500/30">
        <div className="max-w-3xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-white font-bold text-lg leading-tight">Writing Coach</h1>
              <p className="text-blue-300 text-xs">CSS examiner-level feedback</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        {/* Usage banner — shown after results */}
        {usageInfo && phase === 'results' && !usageInfo.isPremium && (
          <div className={`rounded-xl p-3.5 flex items-center justify-between gap-3 border ${
            ...
            {usageInfo.isAnon ? (
              <>
                <p className="text-sm text-blue-800"><span className="font-semibold">Sign in</span> to save your free tries across devices · Unlimited with Premium</p>
                <button onClick={() => router.push('/signin?next=/css/essay-grader')} className="text-xs bg-blue-600 text-white px-3 py-1.5 rounded-lg font-semibold hover:bg-blue-700 transition-colors flex-shrink-0">
                  Sign In
                </button>
              </>
            ) : (
          ...
          <>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex border-b border-gray-100">
                {([
                  { key: 'essay',       label: 'Essay',       icon: FileText,     meta: '1000–1200 words' },
                  { key: 'precis',      label: 'Précis',      icon: BookOpen,     meta: '⅓ of original' },
                  { key: 'long-answer', label: 'Long Answer', icon: ClipboardList, meta: 'Subject papers' },
                ] as const).map(({ key, label, icon: Icon, meta }) => (
                  <button
                    key={key}
                    onClick={() => setMode(key)}
              ...
              <div className="p-4 space-y-4">
                ...
                {mode === 'essay' && (
                  <>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Essay Topic</label>
                      <input
                        type="text"
                        value={essayTopic}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full tabular-nums ${
                          wordCount(essayContent) >= 1000 && wordCount(essayContent) <= 1200 ? 'bg-green-100 text-green-700' :
                          wordCount(essayContent) >= 800 ? 'bg-amber-100 text-amber-700' :
                          wordCount(essayContent) >= 200 ? 'bg-blue-100 text-blue-700' :
                          'bg-gray-100 text-gray-400'
                        }`}>
                          {wordCount(essayContent)} / 1000–1200 words
                        </span>
                      </div>
                      <textarea
                        value={essayContent}
                        ...
                        {(() => {
                          ...
                          return (
                            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full tabular-nums ${
                              ...
                              target && wc >= Math.round(target * 0.85) ? 'bg-amber-100 text-amber-700' :
                              'bg-gray-100 text-gray-400'
                            }`}>
                  ...
                  <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        >
                          {CSS_SUBJECTS.map(s => <option key={s}>{s}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Marks</label>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full tabular-nums ${
                          wordCount(laContent) >= (LA_MIN_WORDS[laMarks] ?? 100) ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'
                        }`}>
            ...
            {!canSubmit() && submitHint() && (
              <p className="text-center text-xs text-gray-400">{submitHint()}</p>
            )}
            {canSubmit() && !usageInfo && (
              <p className="text-center text-xs text-gray-400">1 free try per mode · Sign in to save your quota across devices · Unlimited with Premium</p>
            )}
          </>
        )}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm py-12 px-6 flex flex-col items-center gap-5">
            ...
            <div className="flex gap-1.5">
              {LOADING_STEPS.map((_, i) => (
                <span key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${i <= loadingStep ? 'bg-blue-500' : 'bg-gray-200'}`} />
              ))}
            </div>
          </div>
        )}
        ...
        {phase === 'results' && result && (
          <>
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleReset} className="text-sm text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1">
            ...
            {mode === 'precis' ? (
              <PrecisChecklist
                ...
                wordCount={(result.wordCount as { submitted: number; original: number; target: number; passed: boolean }) || { submitted: 0, original: 0, target: 0, passed: false }}
                ...
                {result.annotations && (result.annotations as unknown[]).length > 0 && (
                  <>
                    ...
                    <AnnotatedEssay
                      ...
                      annotations={result.annotations as Array<{ quote: string; type: 'strength' | 'weakness' | 'suggestion'; comment: string }>}
                    />
                  </>
                )}
              </>
