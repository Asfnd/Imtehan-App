...
export default function FSCChapterPage() {
              ...
              <div className="space-y-2">
                {setsInBatch.map((setNum) => {
                  ...
                  return (
                    <button
                      key={setNum}
                      onClick={() => router.push(`/fsc/${subject}/${encodeURIComponent(chapter)}/set/${setNum}`)}
                      className="w-full text-left px-4 py-3 rounded-xl border border-slate-100 bg-slate-50 hover:bg-emerald-50 hover:border-emerald-200 transition-all group"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${subjectCfg.color} flex items-center justify-center flex-shrink-0`}>
                            <span className="text-white text-sm font-bold">{setNum}</span>
                          </div>
                          <div>
                            <p className="font-semibold text-sm text-slate-900 group-hover:text-emerald-800 transition-colors">Set {setNum}</p>
