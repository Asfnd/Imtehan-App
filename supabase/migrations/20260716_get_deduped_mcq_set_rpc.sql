-- Return exactly one deduped MCQ set page (20 rows) server-side.
-- Replaces Node batches of 400 rows (egress fix). Same stem-dedup rules as quiz-fetcher.

CREATE OR REPLACE FUNCTION public.normalize_mcq_stem(q text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT trim(
    regexp_replace(
      regexp_replace(lower(coalesce(q, '')), '[^a-z0-9\s]', ' ', 'g'),
      '\s+',
      ' ',
      'g'
    )
...
CREATE OR REPLACE FUNCTION public._mcq_table_has_col(p_table text, p_col text)
RETURNS boolean
LANGUAGE sql
STABLE
SET search_path = public
  ...
  IF p_question_needles IS NOT NULL AND cardinality(p_question_needles) > 0 THEN
    ...
    FOREACH v_needle IN ARRAY p_question_needles LOOP
      IF v_needle IS NOT NULL AND btrim(v_needle) <> '' THEN
        v_where := v_where || format('question ILIKE %L OR ', '%' || replace(v_needle, '%', '') || '%');
      END IF;
    END LOOP;
  ...
  v_sql := format(
    ...
    WITH capped AS (
      SELECT
        ...
        t.explanation::text AS explanation
      FROM %I t
      WHERE %s
      ORDER BY t.id
      LIMIT 24000
...
GRANT EXECUTE ON FUNCTION public.normalize_mcq_stem(text) TO public;
