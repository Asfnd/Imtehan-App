#!/usr/bin/env python3
"""
FINAL MCQ Extractor - Fixed all issues
1. Include single-char bold answers (8, 4, A, B, C, D)
2. Improved matching with multiple strategies
3. Confidence scoring for debugging
"""
...
def get_page_bold_texts(pdf, start_page=7):
    """Get bold texts organized by page - FIXED to include single chars"""
    page_bolds = {}
            ...
            for char in chars:
                if 'bold' in char.get('fontname', '').lower():
                    ...
                    if current:
                        ...
                        if text:  # FIXED: was "len(text) > 1" - now includes single chars
                            bolds.append(text.lower())
                        current = []

            if current:
                text = ''.join(current).strip()
                if text:  # FIXED: was "len(text) > 1"
                    bolds.append(text.lower())
...
def find_answer_on_page(options, page_bolds):
    """
    Find answer using multiple matching strategies
    ...
    for bold_text in page_bolds:
        for opt_letter, opt_text in options.items():
            ...
            # Strategy 2: Bold is contained in option or vice versa (for longer texts)
            if len(opt_lower) > 3 and len(bold_text) > 3:
                # Check if one contains the other
                if bold_text in opt_lower or opt_lower in bold_text:
                    # Ensure it's a meaningful match (not just matching "a" in "canada")
                    if len(bold_text) >= len(opt_lower) * 0.5 or len(opt_lower) >= len(bold_text) * 0.5:
                        return opt_letter

            # Strategy 3: Prefix match for long texts
            if len(opt_lower) > 15 and len(bold_text) > 15:
                if opt_lower[:15] in bold_text or bold_text[:15] in opt_lower:
                    return opt_letter

            # Strategy 4: Handle common variations
            # Remove common prefixes/suffixes and compare
            opt_clean = re.sub(r'^(the|a|an)\s+', '', opt_lower)
            bold_clean = re.sub(r'^(the|a|an)\s+', '', bold_text)
            if opt_clean and bold_clean and opt_clean == bold_clean:
                return opt_letter

    return ''


def extract_all_mcqs_final(pdf, start_page=7):
    """Extract MCQs with improved answer detection"""
        ...
        if q_match:
            ...
            if len(options) == 4 and all(k in options for k in ['A', 'B', 'C', 'D']):
                ...
                if q_page is not None:
                    # Check current page and next 2 pages (some questions span pages)
                    relevant_bolds = []
                    for p in [q_page, q_page + 1, q_page + 2]:
                        if p in page_bolds:
...
def process_subject(pdf_path, subject_name, target):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        mcqs = extract_all_mcqs_final(pdf)
        ...
        ans_status = "✅" if ans_pct >= 95 else "⚠️" if ans_pct >= 80 else "❓"

        print(f"\n{count_status} MCQs: {extracted:,} / {target:,} ({pct}%)")
        print(f"{ans_status} Answers: {answers:,} / {extracted:,} ({ans_pct}%)")
...
def main():
    print("="*60)
    print("FINAL MCQ EXTRACTION - All Bugs Fixed")
    print("- Single-char answers included")
    print("- Improved matching strategies")
    print("="*60)
    ...
    for subject, extracted, target, answers in results:
        ...
        print(f"{status} {subject}: {extracted:,} MCQs, {answers:,} answers ({ans_pct}%)")
        total_extracted += extracted
        total_target += target
        total_answers += answers

    print(f"\n{'='*60}")
    total_ans_pct = total_answers * 100 // total_extracted if total_extracted > 0 else 0
    print(f"TOTAL: {total_extracted:,} MCQs, {total_answers:,} answers ({total_ans_pct}%)")
